...
방문자 수와 조회수처럼 댓글도 외부 저장소가 필요한 기능입니다.  
다만 댓글은 단순 카운터보다 사용자 입력, 스팸 방어, 공개 정책, UI 상태가 함께 얽혀 있어서 처음부터 너무 크게 만들지 않기로 했습니다.

이번 버전의 기준은 명확하게 잡았습니다.

- 로그인 없이 이름과 댓글만 입력
- 댓글은 작성 직후 공개
- 답글은 1단계까지만 허용
- 본문은 Markdown 없이 일반 텍스트로 렌더링
- 운영자가 필요하면 DB에서 `hidden_at`을 채워 수동으로 숨김 처리

## Neon DB + Route Handler

저장소는 방문자 수 기능에서 사용하던 Neon PostgreSQL을 그대로 사용했습니다.  
댓글 전용 테이블은 하나만 두고, 답글 관계는 `parent_id` 자기 참조로 표현했습니다.
...
- `GET /api/comments/posts/[slug]` — 포스트별 댓글 목록 조회
- `POST /api/comments` — 댓글 또는 답글 작성

포스트 본문은 여전히 정적으로 렌더링하고, 댓글만 클라이언트 컴포넌트에서 API로 가져오도록 했습니다.  
본문 SEO와 빌드 구조를 유지하면서 동적인 영역만 분리하는 방식입니다.

## 1단계 답글만 허용

처음에는 답글을 아예 뺄까도 고민했는데, 블로그 댓글에서는 짧은 대화가 이어지는 경우가 있을 것 같았습니다.  
대신 스레드형 댓글까지 가면 UI와 관리 정책이 급격히 복잡해지기 때문에 1단계 답글까지만 허용했습니다.

서버에서는 `parent_id`가 들어온 경우 아래 조건을 모두 확인합니다.

- 부모 댓글이 존재해야 한다
- 부모 댓글이 같은 포스트에 속해야 한다
- 부모 댓글이 숨김 처리된 상태가 아니어야 한다
- 부모 댓글 자체가 답글이면 안 된다

```ts
export function validateReplyParentCandidate(postSlug: string, parent: Parent) {
...
클라이언트에서는 댓글 목록을 `CommentThread[]` 형태로 다루도록 했습니다.  
DB에서는 flat row로 읽고, 화면에 넘기기 전에 최상위 댓글과 답글을 묶어 트리처럼 구성합니다.

## 익명성과 스팸 방어

댓글 작성자를 계정으로 식별하지는 않습니다.  
대신 rate limit을 위해 IP 후보와 User-Agent를 `ANALYTICS_SALT`와 함께 해시해서 `author_hash`로 저장합니다.

원본 IP나 User-Agent는 저장하지 않고, 서버에는 해시만 남깁니다.
...
rate limit은 우선 단순하게 잡았습니다.

- 같은 `author_hash` 기준 1분에 3개까지
- 같은 `author_hash` 기준 하루에 30개까지

또 방문자 수 기능과 비슷하게 DNT, prefetch, bot User-Agent 요청은 write를 거부하도록 했습니다.

## 댓글 UI

댓글 본문은 Markdown으로 렌더링하지 않고 일반 텍스트로만 보여줍니다.  
줄바꿈은 `white-space: pre-wrap`으로 보존하고, 긴 단어는 카드 안에서 자연스럽게 줄바꿈되도록 했습니다.

답글 관계는 TOC와 Posts 페이지에서 쓰던 depth 표현을 가져왔습니다.  
단순히 들여쓰기만 하는 대신, 세로선과 가로선으로 댓글과 답글의 관계가 보이도록 했습니다.
