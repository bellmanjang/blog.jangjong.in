 ...
 */
const setEdge = (g: NextSiblingsInPath, p: string, c: string, v: StateVal) => {
    let m = g.get(p);
 ...
 *
 * 구현 포인트
 * - DFS(Article-order)로 child의 inPath를 먼저 계산해야 parent의 inPath를 알 수 있음.
 * - "뒤쪽 형제들(suffix)" 여부는 children을 뒤에서 앞으로 순회하면 suffix 누적값 하나로 O(k)에 계산 가능.
 *
 * - DFS(Article-order): 자식의 path 상태를 먼저 알아야 부모의 path 상태를 계산할 수 있음.
...
export function buildTocLineMeta(
     ...
     * - false: 둘 다 없음
     */
    const dfs = (node: HeadingNode): StateVal => {
...
export function flattenTocIds(nodes: HeadingNode[], out: string[] = []) {
