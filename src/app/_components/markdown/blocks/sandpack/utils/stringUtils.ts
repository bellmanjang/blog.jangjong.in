...
export const isDarkColor = (color: string): boolean => {
    ...
    if (color.startsWith("#")) {
        ...
        r = Number.parseInt(color.substring(1, 2), 16);
        g = Number.parseInt(color.substring(3, 2), 16);
        b = Number.parseInt(color.substring(5, 2), 16);
    } else {
...
export const generateRandomId = (): string => {
    const random = +(Date.now().toString(10).substring(0, 4) + lastCount++);
